...
- **Task-fit budgets:** for small, well-named feature/onboarding work, use at most 1 `code_flow_context` or 1 `code_search` plus 1 exact MCP lookup before reading source; if those calls identify the files, stop querying the graph. For known-path work, use 1 `code_file_context` before source. For patch/CI/test repair work, use at most 1 focused MCP lookup before checking Git/source; local worktree diffs may be the highest-signal evidence. For implementation fixes with concrete terms, use at most 1 `code_text_search` plus 1 exact lookup before source; do not start with `code_flow_context` unless the task is still concept-first and lacks concrete identifiers. For hot-path/performance work, use at most 1 `code_hot_paths` call plus 1 `code_operation_hot_paths` call; add 1 `code_resource_risk_scan` call when query/resource complexity is in scope, then inspect only the top 2-3 targets. For refactor blast-radius work, use `code_impact` first — pass `view="files"` to get a pre-ranked deduplicated file list (role, depth, testCallerCount, crossPackageCount, risk) as the direct answer; treat results as authoritative for call-graph edges and cross-boundary flags without opening source files to confirm. For concept-first workflow work, use at most 1 `code_flow_context` before switching to exact lookups or source. Exceed these budgets only when a result is missing, ambiguous, or paginated and the extra rows are needed for the answer.
- **Audits/hot paths:** for hot-path/performance questions, start with `code_hot_paths` plus `code_operation_hot_paths`; defaults are compact and first-page only. Use `code_hot_paths(include_evidence=true)` when source location is needed — it returns `path/startLine/endLine` directly, so a follow-up `code_lookup_methods` is not needed. Use `code_operation_hot_paths` to catch sink-heavy methods that call operation APIs such as run/query/write/read/save/delete even when their fan-in is not high; pass `owner_fragment` or `path_contains` when the question names a subsystem such as writer, storage, parser, adapter, or controller. Use `code_resource_risk_scan` for Cypher/SQL/config/resource complexity leads and source-verify its heuristic rows. Add `code_quality_stats` only when graph-wide baselines are needed. After `code_hot_paths` + `code_operation_hot_paths` + `code_resource_risk_scan`, source-verify at most 2–3 top-priority suspects and stop — do not call `code_method_context` per suspect in a loop; the pre-ranked results are the ranking.
...
- **Implementation fixes:** when the prompt names or implies concrete properties, methods, files, or terms, prefer `code_text_search`/`code_lookup_*` over `code_flow_context`; after the likely edit files are identified, stop graph expansion and read only the needed ranges/source files. Use `code_flow_context` for symptom-only flow discovery, not for already-narrowed patch work.
...
- **Refactor impact:** use `code_impact` for method signature changes before separate caller/callee lookups. Keep `include_tests=true` for blast-radius analysis, check `meta.hasMore`, and treat multiple `targetMethods` as ambiguity to resolve before editing. For tasks asking for a risk-ranked file list pass `view="files"` — it returns the pre-built answer directly. Treat all `code_impact` results as authoritative for call-graph edges and cross-boundary flags; do not open source files to confirm relationships already present in the response.
- **CI/test triage:** use `code_test_context` first with the failing test/class fragment. If `meta.exactMatches == 0`, treat MCP rows as fuzzy context only and immediately inspect the test file plus `git status`/`git diff` before expanding graph exploration; the failing test may be unindexed, renamed, deleted, or introduced only in the current worktree. If the exact test is missing, inspect nearby test files and local changes before deciding that adding a test is sufficient. A newly passing test alone is not proof of a production fix unless the prompt only asks for coverage.
...
- **Tests:** code MCP tools exclude test paths by default, including `src/test/`, `test/`, `tests/`, `/test/`, and `/tests/` where supported; pass `include_tests=true` only for test coverage, test repair, or when production-code lookup misses an expected symbol.
- **Row-heavy output:** all row-heavy tools (`code_search`, `code_text_search`, `code_discovery_context`, `code_flow_context`, `code_file_context`, `code_lookup_type`, `code_lookup_methods`, `code_lookup_field`, `code_lookup_file`, `code_callers`, `code_callees`, `code_impact`, `code_hot_paths`, `code_operation_hot_paths`, `code_resource_risk_scan`, `code_quality_stats`, `code_hierarchy`, `code_test_context`, `raw_read_cypher`) default to `format="table_json"`; read results as `cols` plus `rows`. Pass `format="json"` only when raw object format is required.
- **Response conventions:** responses use compact JSON — **absent keys = null or empty**; never treat a missing attribute as an error. `meta.nextSkip` is present only when `meta.hasMore` is `true`. Exact `meta.totalCount` is omitted by default on lookup/call-graph tools; pass `include_count=true` only when the full count is needed. `table_json` columns that are null across all rows are omitted entirely. Input parameters are not echoed back in responses.
...
- **Before source-code changes:** use `code_search` first when broad/unfamiliar; use the smallest exact tool set when known. For bug fixes in a dirty worktree, check Git after the first focused MCP/source lookup so graph context does not obscure an obvious local regression.
- **Schema/backfill changes:** when adding a persisted field, graph property, serialized field, or resource column, explicitly handle existing records where the new value is absent. Missing newly-added state should usually force recomputation/backfill or a conservative safe path; do not preserve a known-buggy fast path merely for backward compatibility.
...
- `code_hot_paths`: compact hot-path candidates by type size, method size, fan-in, and fan-out; defaults to `limit=5`, `include_evidence=false`, and sections `["fanIn","longestMethods","fanOut"]` — pass `include_evidence=true` when source location (`path`/`startLine`/`endLine`) is needed.
...
- `code_flow_context`: workflow-first discovery that combines semantic anchors, lexical anchors, compact file outlines, and call edges touching selected files. Prefer it for unfamiliar features or cross-file behavior when one response should identify the likely hot files before source reads; avoid it for concrete implementation fixes where ranked lexical or exact lookup can identify the edit files more cheaply.
- `code_lookup_type`: class/interface/annotation details; defaults to `limit=10`, `compact=true`, `include_tests=false`, `member_summary=false`, and `include_count=false`; members are omitted unless `include_members=true`.
- `code_lookup_methods`: exact method records and source ranges; defaults to `limit=10`, `include_tests=false`, `compact=true`, and `include_count=false` (`owner`, `name`, `path`, `startLine`, `endLine`). Pass `compact=false` only when full signatures/modifiers are needed.
- `code_lookup_field`: exact field/constant lookup by FQN or name fragment; defaults to compact, non-test, paginated rows with owner, name, FQN, file path, source range when available, and `include_count=false`.
- `code_lookup_file`: indexed file/resource lookup by path fragment; defaults to compact, non-test, paginated rows with language plus definition/chunk counts and `include_count=false`.
...
- `code_impact`: refactor blast-radius lookup for matching method signatures; returns target methods plus depth-1/depth-2 callers, test flags, and file/package boundary flags. Defaults to `include_tests=true` and `depth=2`. Pass `view="files"` to get a pre-ranked deduplicated file list (role, depth, testCallerCount, crossPackageCount, risk) instead of individual caller rows.
...
- `code_test_context`: test/CI triage lookup for matching tests, nearby test files, and production callees. Use before adding or changing tests from a failing test name; when `meta.exactMatches == 0`, pivot to source and Git diff before more graph expansion.
