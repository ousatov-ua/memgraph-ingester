...
- **Task-fit budgets:** for small, well-named feature/onboarding work, use at most 1 `code_search` plus 1-2 exact MCP lookups before reading source; if those calls identify the files, stop querying the graph. For hot-path/performance work, use at most 1 `code_hot_paths` call plus `code_method_context` for only the top 2-3 targets before reading source. Exceed these budgets only when a result is missing, ambiguous, or paginated and the extra rows are needed for the answer.
...
- `code_search`: CodeChunk RAG discovery; defaults to 5 non-test, deduped, range-first hits (`kind`, `owner`, `name`, `path`, `startLine`, `endLine`, `score`). Text is omitted unless `include_text=true`.
