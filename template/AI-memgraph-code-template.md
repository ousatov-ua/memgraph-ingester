...
- **Response conventions:** responses use compact JSON — **absent keys = null or empty**; never treat a missing attribute as an error. `meta.nextSkip` is present only when `meta.hasMore` is `true`. `table_json` columns that are null across all rows are omitted entirely. Input parameters are not echoed back in responses.
