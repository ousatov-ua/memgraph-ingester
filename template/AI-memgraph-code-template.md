...
- Regular and watch re-ingestion prune graph state for deleted source files and declarations removed from changed files. File-local outgoing `CALLS`, `ANNOTATED_WITH`, `EXTENDS`, and `IMPLEMENTS` relationships are refreshed before current file data is written, while current method nodes are preserved so incoming `CALLS` edges from unchanged files survive incremental runs.
