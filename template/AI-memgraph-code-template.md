...
- **Broad/unfamiliar code:** use 1 concise `code_discovery_context` query first; use `code_search` only when raw RAG anchors are enough, and use `code_text_search` when the concept includes concrete words likely present in code/comments/templates. Defaults are compact (`limit=3-5`, `include_text=false`). Treat RAG/text hits as discovery only; verify with exact MCP lookup or source before making claims.
- **Known targets:** skip RAG and use exact tools with precise fragments and compact defaults: `code_lookup_type`, `code_lookup_methods`, `code_lookup_field`, `code_lookup_file`, `code_callers`, `code_callees`, `code_impact`, `code_test_context`, and `code_hierarchy`.
...
- **Fields/config/resource files:** use `code_lookup_field` for constants, config fields, and member variables; use `code_lookup_file` for indexed source/resource paths, including templates and Cypher files, before falling back to grep/glob.
...
- `code_search`: CodeChunk RAG discovery; defaults to 5 non-test, deduped, range-first hits (`kind`, `owner`, `name`, `path`, `startLine`, `endLine`, `score`). Text and source keys are omitted unless `include_text=true` or `include_keys=true`. Supports compact filters such as `kinds`, `path_prefixes`, `path_contains`, `owner_fragment`, and `min_score`.
- `code_text_search`: lexical search over indexed chunk text/path/source ids; use when concrete terms are likely present and grep would otherwise be used. Defaults to compact rows and omitted text.
